...
let TokyoLorem = class extends s {
  constructor() {
    super(...arguments);
    this.word = 5;
    this.paragraph = 1;
  ...
  generateLorem() {
    let _lorem = this.language === "en" ? englishProductWords : arabicProductWords;
    let text = _lorem[0];
    let _loremIndex = 0;
    let getLoremIndex = () => {
      _loremIndex += 1;
    ...
    for (let i2 = 0; i2 < this.word - 1; i2++) {
  ...
  render() {
    return $`<div dir="auto">
      ${Array(this.paragraph).fill("").map(() => $`<p>${this.generateLorem()}</p>`)}
    </div>`;
  }
};
__decorateClass([
  e({ type: Number })
], TokyoLorem.prototype, "word", 2);
__decorateClass([
  e({ type: Number })
], TokyoLorem.prototype, "paragraph", 2);
__decorateClass([
  e({ type: String })
], TokyoLorem.prototype, "language", 2);
TokyoLorem = __decorateClass([
  n$1("tokyo-lorem")
], TokyoLorem);
export { TokyoLorem };
